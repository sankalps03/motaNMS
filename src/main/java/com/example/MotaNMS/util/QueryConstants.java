...
public class QueryConstants
{
  ...
  public static final String SSH_LATEST_DATA_QUERY ="SELECT MAX(CASE WHEN METRICTYPE='ping.packet.rtt' THEN METRICVALUE END) AS ping_packet_rtt,MAX(CASE WHEN METRICTYPE='cpu.percent.total' THEN METRICVALUE END) AS cpu_percent_total,MAX(CASE WHEN METRICTYPE='disk.percent.used' THEN METRICVALUE END) AS disk_percent_used,MAX(CASE WHEN METRICTYPE='memory.percent.used' THEN METRICVALUE END) AS memory_percent_used FROM polling p WHERE IPADDRESS= ? AND METRICTYPE IN ('ping.packet.rtt','cpu.percent.total','disk.percent.used','memory.percent.used') AND TIMESTAMP=(SELECT MAX(TIMESTAMP) FROM polling WHERE IPADDRESS= ? AND METRICTYPE=p.METRICTYPE);";
  ...
  public static final String UPDATE_AVAILABILITY_STATUS_QUERY ="UPDATE monitor m SET m.STATUS=CASE WHEN m.IPADDRESS IN (SELECT DISTINCT p.IPADDRESS FROM polling p WHERE p.METRICTYPE='ping.packet.loss' AND p.METRICVALUE < 100 AND p.TIMESTAMP>=NOW() - INTERVAL '5' MINUTE) THEN 'up' ELSE 'down' END;";
