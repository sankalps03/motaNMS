...
public class QueryConstants {
  ...
  public static final String SSH_LATEST_DATA_QUERY ="SELECT p.METRICTYPE, p.METRICVALUE FROM polling p WHERE p.IPADDRESS = '?' AND p.METRICTYPE IN ('ping.packet.sent', 'ping.packet.rcv', 'ping.packet.rtt', 'cpu.percent.total', 'disk.percent.used','memory.percent.used','ping.packet.loss') AND p.TIMESTAMP = (SELECT MAX(TIMESTAMP) FROM polling WHERE IPADDRESS = '?' AND METRICTYPE = p.METRICTYPE )";

  public static final String PING_LATEST_DATA_QUERY ="SELECT p.METRICTYPE, p.METRICVALUE FROM polling p WHERE p.IPADDRESS = '?'AND p.METRICTYPE IN ('ping.packet.sent', 'ping.packet.rcv', 'ping.packet.rtt','ping.packet.loss') AND p.TIMESTAMP = ( SELECT MAX(TIMESTAMP) FROM polling WHERE IPADDRESS = '?' AND METRICTYPE = p.METRICTYPE )";
